...
static void mptcp_v4_subflows(struct sock *meta_sk,
		const struct mptcp_loc4 *loc, struct mptcp_rem4 *rem, int num_to_create)
{
	int i;
	printk(KERN_INFO "******** Entering %s num_to_create is %d ********\n",__func__,num_to_create);
	for (i = 0; i < num_to_create; i++) {
		printk(KERN_INFO "******** in %s i = %d ********\n",__func__,i);
...
static int cal_num_subflow(int i) 
{
	int j;
	int k = 0;
	int n = num_subflows;
...
static void create_subflow_worker(struct work_struct *work)
{
	...
	printk(KERN_INFO "******** Current parameter is %d ********\n",num_subflows);
	printk(KERN_INFO "******** Arriving at next_subflow ********\n");
